...
<head>
    ...
    <style>
        ...
        .login-success{background:rgba(12,209,62,.12);border:1px solid rgba(12,209,62,.35);border-radius:5px;color:#5dfc8a;font-size:13px;padding:10px 16px;margin:16px 20px 0;text-align:left}
...
<body class="min-vh-100" style="background:url('<?= BASE_URL ?>/app/uploads/gymrat.jpg') center/cover no-repeat;">
    ...
    <div class="wrapper fadeInDown">
        <div id="formContent">
            ...
            <?php if (!empty($justRegistered) && empty($error)): ?>
                <div class="login-success"><i class="bi bi-check-circle me-1"></i>Account created! Now log in below.</div>
            <?php endif; ?>
            <?php if (!empty($error)): ?>
                <div class="login-error"><i class="bi bi-exclamation-circle me-1"></i><?= htmlspecialchars($error) ?></div>
            <?php endif; ?>
            <form action="<?= BASE_URL ?>/auth/login" method="post">
                <input type="text"     id="username" class="fadeIn second" name="username" placeholder="username or email" value="<?= htmlspecialchars($prefillUsername ?? '') ?>" autocomplete="username">
                <input type="password" id="password" class="fadeIn third"  name="password" placeholder="password" autocomplete="current-password">
                <input type="submit"   id="loginBtn" class="fadeIn fourth" value="Log In">
            </form>
            <div id="formFooter"><a class="underlineHover" href="<?= BASE_URL ?>/auth/register">Sign Up</a></div>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <?php if (!empty($justRegistered)): ?><script>document.getElementById('password').focus();</script><?php endif; ?>
</body>
</html>